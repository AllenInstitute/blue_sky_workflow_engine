...
_WORKFLOW_NAME = 'mock_workflow'
#_WORKFLOW_NAME = 'em_2d_montage'
...
class IngestTileSetModule(argschema.ArgSchemaParser):
    ...
    def run(self):
        ...
        if is_lc:
            ...
            with open('tileset.uid') as f:
                body['reference_set_id'] = f.read()
        ...
        self.logger.info("enqueued object: %s",
            str(response['enqueued_object_uid']))

        if is_lc:
            with open('tileset.uid', 'w') as f:
                f.write(response['enqueued_object_uid'])
