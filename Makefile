...
doc: FORCE
	sphinx-apidoc -d 4 --force -H "Blue Sky Workflow Engine" -A "Allen Institute for Brain Science" -V $(VERSION) -R $(VERSION)$(RELEASE) --full -o doc --module-first workflow_client
	sphinx-apidoc -d 4 --force -H "Blue Sky Workflow Engine" -A "Allen Institute for Brain Science" -V $(VERSION) -R $(VERSION)$(RELEASE) --full -o doc --module-first $(PROJECTNAME)
